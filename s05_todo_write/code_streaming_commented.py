"""s05.5: 在 s05 的 Todo Agent 上改用 LangGraph streaming 逐步打印。
...
5. 用 agent.stream(..., stream_mode="values") 边执行边打印 todo、工具调用和模型回复。
"""
...
# 执行一轮 Agent 对话；s05.5 使用 stream，所以每个图步骤结束后就能打印新增内容。
def agent_loop(messages: list) -> None:
    # seen 记录已经打印到第几条消息，避免 streaming 时重复打印。
    seen = len(messages)
    # final_state 保存最后一次 stream 出来的完整 state，循环结束后用它更新历史。
    final_state = None
    # last_todos 用来判断 todo 是否变化，避免同一份 todo 反复刷屏。
    last_todos = None

    # stream_mode="values" 会在每个步骤后吐出当前完整 state。
    # 这样可以边执行边看到 todo、工具调用和模型回复，而不是最后才翻记录。
    for state in agent.stream({'messages': messages}, stream_mode='values'):
        # 每次都更新 final_state，循环结束后它就是最终状态。
        final_state = state

        # TodoListMiddleware 的 todo 状态会随着 state 一起流出来。
        todos = state.get("todos")
        # 只有 todo 发生变化时才打印，减少重复输出。
        ...
        # 当前 state 中的 messages 包含历史消息和本步骤新增消息。
        ...
        # 更新已打印位置，为下一次 stream chunk 做准备。
        seen = len(current_messages)

    # 如果本轮确实执行过，就用最终 state 更新外部历史。
    if final_state is not None:
        # 切片赋值可以保留调用方传进来的 history 列表对象。
        messages[:] = final_state['messages']

# 只有直接运行 python s05.5_commented.py 时，下面这段命令行交互才会执行。
